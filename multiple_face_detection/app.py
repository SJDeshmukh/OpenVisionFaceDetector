...
def get_realtime_engine():
    global _mesh_engine
    print(f"[DEBUG] get_realtime_engine ENTRY, _mesh_engine is {type(_mesh_engine)}", flush=True)
    ...
    print(f"[DEBUG] get_realtime_engine - target mesh_dir: {mesh_dir}", flush=True)
    if not os.path.isdir(mesh_dir):
        print(f"[DEBUG] get_realtime_engine - not found, trying current dir", flush=True)
        mesh_dir = os.path.join(_BASE_DIR, "standalone_live_mesh")
        
    if os.path.isdir(mesh_dir):
        mesh_parent = os.path.dirname(mesh_dir)
        print(f"[DEBUG] get_realtime_engine - mesh_dir found, adding mesh_parent to sys.path: {mesh_parent}", flush=True)
        if mesh_parent not in sys.path:
            sys.path.append(mesh_parent)
        try:
            print(f"[DEBUG] get_realtime_engine - attempting import from standalone_live_mesh.inference", flush=True)
            from standalone_live_mesh.inference import get_realtime_engine as _get
            print(f"[DEBUG] get_realtime_engine - import success, calling _get()", flush=True)
            _mesh_engine = _get()
            print(f"[DEBUG] get_realtime_engine - _get() returned: {type(_mesh_engine)}", flush=True)
            return _mesh_engine
        ...
        print(f"[DEBUG] get_realtime_engine - mesh_dir NOT found", flush=True)
    return None
...
class GFPGANManager:
    ...
    def enhance_crop(self, crop_rgb: np.ndarray, upscale: int = 2, whole: bool = False, fidelity: float = 0.5, landmarks: np.ndarray = None) -> np.ndarray:
        ...
        if min(h_c, w_c) < 128:
            try:
                # Use local/global RealESRGAN if available
        ...
        try:
            ...
            if fidelity < 1.0:
                ...
                if mask is not None and fidelity < 0.8:
                    ...
                    # Standard global blend or pure AI output if fidelity is high
                    # If fidelity is high (>=0.8), we trust the AI more to define the edges clearly
...
class MTCNNManager:
    ...
    def load(self):
        ...
        try:
            ...
            # Aggressively force CPU for MTCNN to avoid MPS pooling bugs
            self._device = torch.device("cpu")
            self._model = MTCNN(keep_all=True, device=self._device, post_process=False)
            self._model.to(self._device)
            print(f"MTCNN model loaded on {self._device}")
            return self._model
        except Exception as e:
            print(f"Error loading MTCNN: {e}")
            return None

    def detect(self, rgb_image: np.ndarray, landmarks=False) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
        m = self.load()
        if m is None:
            if landmarks: return None, None, None
            return None, None
        try:
            # MTCNN expects RGB images
            if landmarks:
                boxes, probs, points = m.detect(rgb_image, landmarks=True)
                if boxes is None:
                    return np.zeros((0, 4), dtype=np.float32), np.zeros((0,), dtype=np.float32), np.zeros((0, 5, 2), dtype=np.float32)
                return boxes.astype(np.float32), probs.astype(np.float32), points.astype(np.float32)
            else:
                boxes, probs = m.detect(rgb_image)
                if boxes is None:
    ...
    def detect_faces(self, rgb_image: np.ndarray) -> list:
        """Returns a list of dicts for compatibility with RealTimeEngine/inference.py"""
        boxes, probs, points = self.detect(rgb_image, landmarks=True)
        ...
        for i in range(len(boxes)):
            ...
            results.append({
                'box': [int(box[0]), int(box[1]), int(box[2]), int(box[1]), int(box[2]-box[0]), int(box[3]-box[1])],
...
def get_mtcnn_manager():
    global _mtcnn_manager
    if _mtcnn_manager is None:
        _mtcnn_manager = MTCNNManager()
    return _mtcnn_manager
...
def get_retina_det():
    ...
    if _retina_det is None:
        try:
            ...
            _retina_det = init_detection_model('retinaface_resnet50', half=False, device=get_device())
        except Exception as e:
            print(f"RetinaFace init error: {e}", flush=True)
...
def _detect_boxes_with_downscale(bgr_image: np.ndarray, max_side: int = 1280):
    ...
    # 1. Try MTCNN as default (per user request)
    mtcnn_mgr = get_mtcnn_manager()
    try:
        rgb_small = cv2.cvtColor(bgr_small, cv2.COLOR_BGR2RGB)
        boxes_s, scores, points_s = mtcnn_mgr.detect(rgb_small, landmarks=True)
        if boxes_s is not None and len(boxes_s) > 0:
            if scale != 1.0:
                boxes_s = (boxes_s.astype(np.float32) / scale).astype(np.float32)
                points_s = (points_s.astype(np.float32) / scale).astype(np.float32)
            
            # Convert points to keypoints dict list
            kpts_list = []
            ...
            return boxes_s, scores, kpts_list
    except Exception as e:
        print(f"[MTCNN] Detection failed: {e}")

    # If LOW_RAM_MODE is enabled, we stop here to avoid loading RetinaFace or other models.
    if LOW_RAM_MODE:
        return np.zeros((0, 4), dtype=np.float32), np.zeros((0,), dtype=np.float32), []

    # 2. Fallback to RetinaFace
    rd = get_retina_det()
    if rd is not None:
        try:
            # RetinaFace from facexlib returns a numpy array: [n, 15]
            # [x1, y1, x2, y2, score, x_le, y_le, x_re, y_re, x_n, y_n, x_ml, y_ml, x_mr, y_mr]
            obj = rd.detect_faces(bgr_small, conf_threshold=0.5)
            if obj is not None and len(obj) > 0:
                boxes_s = obj[:, :4].astype(np.float32)
                scores = obj[:, 4].astype(np.float32)
                kpts_list = []
                for i in range(len(obj)):
                    pts = obj[i, 5:] # 10 elements
                    kpts = {
                        'left_eye': [float(pts[0]), float(pts[1])],
                        'right_eye': [float(pts[2]), float(pts[3])],
                        'nose': [float(pts[4]), float(pts[5])],
                        'mouth_left': [float(pts[6]), float(pts[7])],
                        'mouth_right': [float(pts[8]), float(pts[9])]
                    }
                    kpts_list.append(kpts)
                
                if scale != 1.0:
                    boxes_s = (boxes_s / scale).astype(np.float32)
                    for k in kpts_list:
                        for key in k:
                            k[key][0] /= scale
                            k[key][1] /= scale
    ...
    # 3. Last fallback to SDK (SSD) - Note: SDK might not return landmarks easily
    try:
...
def _compute_centered_box(x1, y1, x2, y2, w, h, scale: float = 1.2):
    ...
    # Ensure square-ish aspect ratio and enough padding for internal detectors
    if scale < 1.5:
        scale = 1.5
    side = int(max(face_w, face_h) * scale)
...
def _compute_tight_box(x1, y1, x2, y2, w, h, padding=3):
    """Computes a tight box with fixed pixel padding around the detector box."""
    x1n = max(0, x1 - padding)
    y1n = max(0, y1 - padding)
    x2n = min(w, x2 + padding)
    y2n = min(h, y2 + padding)
    return x1n, y1n, x2n, y2n
...
def detect_faces(image_input, enhancer: str = "GFPGAN", enhance_level: float = 0.5, gfpgan_upscale: int = 2, codeformer_w: float = 0.5, compute_embeddings: bool = False, crop_mode: str = "Face", portrait_scale: float = 3.0, preclean_whole: bool = False, preclean_level: float = 0.4, det_max_side: int = 1280):
    ...
    boxes, scores, facial_landmarks = _detect_boxes_with_downscale(bgr, max_side=int(det_max_side))
    if boxes is None:
        return None, [], pd.DataFrame([]), pd.DataFrame([])
    ...
    for i in range(len(boxes)):
        ...
        if crop_mode == "Portrait":
            ...
            tx1, ty1, tx2, ty2 = _compute_tight_box(x1, y1, x2, y2, w, h, padding=3)
            anns.append(([tx1, ty1, tx2, ty2], f"{s:.2f}"))
            crop = rgb_for_det[ty1:ty2, tx1:tx2]
        else:
        ...
        if crop.size > 0:
            ...
            # CRITICAL: Always use a scaled 1.5x crop for embeddings to ensure consistency and context
            f1x1, f1y1, f1x2, f1y2 = _compute_centered_box(bx1, by1, bx2, by2, w, h, scale=1.5)
            ...
            if enhancer == "None":
                ...
                # Use enhance_level as fidelity for GFPGAN (scale 0-1)
                fidelity_val = max(0.5, float(enhance_level))
                out_crop = get_gfpgan_manager().enhance_crop(temp_crop, upscale=1, whole=(crop_mode == "Portrait"), fidelity=fidelity_val)
            ...
            if compute_embeddings:
                ...
                if enhancer != "None" and face_only_tight.size > 0:
                    # Match backend routes: upscale=2, fidelity=0.8, and pass landmarks for alignment
                    # Note: We use upscale=2 here because that's what faces.py does for pure_face thumbnails/embeddings
                    # We increased fidelity from 0.5 to 0.8 for better sharpness
                    emb_crop = get_gfpgan_manager().enhance_crop(face_only_tight, upscale=2, whole=False, fidelity=0.8, landmarks=lmks_local_emb)
    ...
    print(f"[RE-ENGINE] detect_faces REACHED 3D SECTION. boxes: {len(boxes)}", flush=True)
    landmarks_3d_list = []
    struct_vec_list = []
    
    # In LOW_RAM_MODE, we skip 3D coordinates entirely to save memory and CPU
    if LOW_RAM_MODE:
        print("[RE-ENGINE] LOW_RAM_MODE: Skipping 3D landmarks extraction.", flush=True)
        ...
        if engine is not None:
            for i in range(len(boxes)):
                ...
                try:
                    ...
                    face_for_3d = rgb[c3y1:c3y2, c3x1:c3x2]
                    if face_for_3d.size > 0:
                        # LOCAL REFINEMENT: Localize keypoints for the crop
                        lmks_local = {}
                        if facial_landmarks and i < len(facial_landmarks):
                            kpts = facial_landmarks[i] # In _detect_boxes_with_downscale, this is the kpts dict itself
                            if isinstance(kpts, dict):
                                for k, pos in kpts.items():
                                    lmks_local[k] = [pos[0] - c3x1, pos[1] - c3y1]
                        
                        lmks_list = engine.extract_landmarks(face_for_3d, landmarks_override=lmks_local if lmks_local else None)
                    ...
                    print(f"[RE-ENGINE] Exception in 3D extraction: {e}", flush=True)
                    landmarks_3d_list.append([])
                    struct_vec_list.append([])
        else:
            landmarks_3d_list = [[] for _ in range(len(boxes))]
    ...
    with gr.Blocks(title="Face Detection (Faceplugin SDK)") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                crop_mode = gr.Radio(choices=["Face", "Tight", "Portrait"], value="Face", label="Crop mode")
