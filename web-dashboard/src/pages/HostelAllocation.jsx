...
export default function HostelAllocation() {
  ...
  const removeResident = async resident => {
    const reason = window.prompt(`Reason for removing ${resident.name} from their bed:`, 'Departure');
  ...
  return <div className="space-y-4">
    ...
    <div className="grid min-h-[650px] grid-cols-1 gap-4 xl:grid-cols-[260px_minmax(0,1fr)_300px]">
      <aside className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="border-t pt-4"><label className="text-xs font-semibold uppercase text-slate-500">{search ? 'Resident search results' : 'Unallocated residents'}</label><div className="relative my-2"><Search className="absolute left-2.5 top-2.5 text-slate-400" size={15} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Resident, ID, building or room" className="w-full rounded-lg border py-2 pl-8 pr-2 text-sm" /></div>
        ...
        {!building ? <div className="grid h-full min-h-[500px] place-items-center text-center"><div><Building2 className="mx-auto mb-3 text-slate-300" size={52} /><h2 className="text-lg font-bold">{data.permissions.can_manage_buildings ? 'Create your first hostel building' : 'No assigned hostel buildings'}</h2><p className="mb-4 text-sm text-slate-500">Building → Floors → Rooms → Beds</p>{data.permissions.can_manage_buildings && <button onClick={createBuilding} className="rounded-lg bg-indigo-600 px-4 py-2 text-white">Create Building</button>}</div></div> : <>
          ...
          <div className="origin-top-left space-y-4" style={{ transform: `scale(${zoom})`, width: `${100 / zoom}%` }}>{building.floors.map(floor => <section id={`floor-${floor.id}`} key={floor.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {!collapsed.has(floor.id) && <div className="grid grid-cols-1 gap-3 p-3 md:grid-cols-2 2xl:grid-cols-3">{floor.rooms.filter(room => roomMatchesFilter(room, roomFilter) && (!search.trim() || `${building.name} ${floor.name} ${room.room_number} ${room.beds.map(bed => `${bed.resident?.name || ''} ${bed.resident?.resident_id || ''}`).join(' ')}`.toLowerCase().includes(search.toLowerCase()))).map(room => <article key={room.id} draggable={mode === 'layout'} onDragStart={() => setDraggedRoom({ roomId: room.id, floorId: floor.id })} onDragOver={e => { if (mode === 'layout' || (mode === 'allocation' && room.summary.available)) e.preventDefault(); }} onDrop={e => { e.preventDefault(); if (mode === 'layout') dropRoom(floor, room.id); else beginRoomAssignment(e.dataTransfer.getData('residentId'), { ...room, floor, building }); }} onClick={() => setSelectedRoomId(room.id)} className={`rounded-xl border-2 bg-white p-3 transition ${selectedRoomId === room.id ? 'border-indigo-500 shadow-md' : room.summary.available ? 'border-emerald-200' : room.summary.unavailable === room.summary.total ? 'border-slate-300' : 'border-blue-200'}`}>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{room.beds.map(bed => <button key={bed.id} onClick={event => { event.stopPropagation(); setSelectedBedId(bed.id); if (mode === 'allocation' && selectedResidentId && bed.status === 'available') beginAssignment(selectedResidentId, { ...bed, room }); else if (bed.resident) locateResident(data.residents.find(item => item.id === bed.resident.id) || bed.resident); }} onDragOver={event => { if (mode === 'allocation' && bed.status === 'available') event.preventDefault(); }} onDrop={event => { event.preventDefault(); event.stopPropagation(); beginAssignment(event.dataTransfer.getData('residentId'), { ...bed, room }); }} className={`min-h-16 rounded-lg border p-2 text-left text-xs focus:ring-2 focus:ring-indigo-500 ${STATUS_STYLE[bed.status]}`} aria-label={`Room ${room.room_number}, ${bed.bed_label}, ${bed.status}${bed.resident ? `, occupied by ${bed.resident.name}` : ''}`}><span className="flex items-center gap-1 font-semibold"><BedDouble size={13} />{bed.bed_label}</span>{bed.resident ? <span className="mt-1 block truncate">{bed.resident.name}</span> : <span className="mt-1 block capitalize">{bed.status === 'available' ? 'Available' : bed.status}</span>}</button>)}</div>
      ...
      <aside className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        {selectedResident ? <div className="space-y-3"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-indigo-100 font-bold text-indigo-700">{initials(selectedResident.name)}</span><div><h3 className="font-bold">{selectedResident.name}</h3><p className="text-xs text-slate-500">Resident ID {selectedResident.resident_id}</p></div></div><dl className="space-y-2 text-sm"><div><dt className="text-xs text-slate-400">Gender</dt><dd>{selectedResident.gender || 'Requires review'}</dd></div><div><dt className="text-xs text-slate-400">Academic year</dt><dd>{selectedResident.academic_year || 'Requires review'}</dd></div><div><dt className="text-xs text-slate-400">Category</dt><dd>{selectedResident.resident_category || 'Requires review'}</dd></div></dl>{selectedResident.allocation ? <><p className="rounded-lg bg-blue-50 p-2 text-xs text-blue-700">Currently: {selectedResidentBed ? `${selectedResidentBed.building.name} / ${selectedResidentBed.floor.name} / Room ${selectedResidentBed.room.room_number} / ${selectedResidentBed.bed_label}` : `Bed #${selectedResident.allocation.bed_id}`}. Select another available bed to transfer.</p>{data.permissions.can_allocate && <button onClick={() => removeResident(selectedResident)} className="w-full rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600">Remove allocation</button>}</> : <p className="rounded-lg bg-emerald-50 p-2 text-xs text-emerald-700">Select or drag this resident onto an available bed.</p>}</div> : selectedRoom ? <div className="space-y-3"><div><h3 className="text-lg font-bold">Room {selectedRoom.room_number}</h3><p className="text-sm text-slate-500">{selectedRoom.room_type}</p></div><Summary value={selectedRoom.summary} /><p className="text-xs text-violet-700">{ruleText(selectedRoom.effective_eligibility) || 'No eligibility restrictions'}</p>{selectedBed && <div className={`rounded-lg border p-2 text-xs ${STATUS_STYLE[selectedBed.status]}`}><strong>{selectedBed.bed_label}: <span className="capitalize">{selectedBed.status}</span></strong>{selectedBed.unavailable_reason && <div>Reason: {selectedBed.unavailable_reason.replaceAll('_', ' ')}</div>}{selectedBed.unavailable_note && <div>Note: {selectedBed.unavailable_note}</div>}{selectedBed.unavailable_from && <div>Unavailable from: {String(selectedBed.unavailable_from)}</div>}{selectedBed.expected_reopening_date && <div>Expected reopening (review required): {String(selectedBed.expected_reopening_date)}</div>}{selectedBed.reservation_expires_at && <div>Reservation expires: {String(selectedBed.reservation_expires_at)}</div>}</div>}{mode === 'layout' && data.permissions.can_edit_layout && <div className="space-y-2"><button onClick={() => editRoom(selectedRoom)} className="w-full rounded-lg bg-indigo-600 px-3 py-2 text-sm text-white">Edit room and capacity</button>{data.permissions.can_manage_eligibility && <button onClick={() => editEligibility(selectedRoom)} className="w-full rounded-lg border px-3 py-2 text-sm">Eligibility rules</button>}<button onClick={() => deleteRoom(selectedRoom)} className="w-full rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600"><Trash2 size={14} className="inline" /> Delete vacant room</button></div>}{selectedBed && mode === 'layout' && <button onClick={() => configureBed(selectedBed)} className="w-full rounded-lg border px-3 py-2 text-sm"><Wrench size={14} className="inline" /> Configure selected bed</button>}</div> : <div className="py-12 text-center text-slate-400"><Home className="mx-auto mb-2" /><p className="text-sm">Select a room or resident to see details and actions.</p></div>}
