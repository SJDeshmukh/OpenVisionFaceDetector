      ...
      <aside className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        {selectedResident ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-indigo-100 font-bold text-indigo-700">{initials(selectedResident.name)}</span><div><h3 className="font-bold">{selectedResident.name}</h3><p className="text-xs text-slate-500">Resident ID {selectedResident.resident_id}</p></div></div>
          ...
          <div className="space-y-3">
            ...
            <Summary value={selectedRoom.summary} />
            <p className="text-xs text-violet-700">{ruleText(selectedRoom.effective_eligibility) || 'No eligibility restrictions'}</p>
            {selectedBed && <div className={`rounded-lg border p-2 text-xs ${STATUS_STYLE[selectedBed.status]}`}><strong>{selectedBed.bed_label}: <span className="capitalize">{selectedBed.status}</span></strong>{selectedBed.unavailable_reason && <div>Reason: {selectedBed.unavailable_reason.replaceAll('_', ' ')}</div>}{selectedBed.unavailable_note && <div>Note: {selectedBed.unavailable_note}</div>}{selectedBed.unavailable_from && <div>Unavailable from: {String(selectedBed.unavailable_from)}</div>}{selectedBed.expected_reopening_date && <div>Expected reopening (review required): {String(selectedBed.expected_reopening_date)}</div>}{selectedBed.reservation_expires_at && <div>Reservation expires: {String(selectedBed.reservation_expires_at)}</div>}</div>}
