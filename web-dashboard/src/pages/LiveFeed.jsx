...
const DeviceMonitor = ({ vendorId, deviceId, deviceName, frame }) => {
  ...
  const status = frame && now - frame.ts < 1500 ? 'online' : 'offline';
...
const LiveFeed = () => {
  ...
  useEffect(() => {
    ...
    const handleFrameUpdate = (ev) => {
      if (!ev || ev.vendor_id == null || ev.device_id == null || !ev.image) return;
      const key = `${ev.vendor_id}-${ev.device_id}`;
      const img = ev.image.startsWith('data:') ? ev.image : `data:image/jpeg;base64,${ev.image}`;
      setFrames(prev => ({ ...prev, [key]: { image: img, ts: Date.now() } }));
    };
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                          {displayedDevices.map(dev => (
                              <DeviceMonitor
                                  ...
                                  frame={frames[`${dev.vendor_id}-${dev.device_id}`]}
                              />
                          ))}
                      </div>
                  </div>
