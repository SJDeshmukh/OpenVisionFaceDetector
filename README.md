...
### XChat Phase 1 (AWS Bedrock Nemotron, Gemini, Mistral, Groq, Cerebras fallback, or OmniRoute)
...
On an existing EC2 deployment, choose AWS Bedrock, Gemini, Mistral, Groq, OmniRoute, or disabled and securely configure the selected provider with hidden input:

```bash
bash setup_aws.sh configure-ai
```

Fresh deployments also ask which XChat provider to activate. The script stores the selected key in the protected application environment with mode `0600`, reuses it on later runs, and checks the key and model before interrupting the running application. A temporary provider `429` or `5xx` response is reported but does not block deployment; invalid primary credentials still stop setup. Runtime logs include a per-request trace ID, selected model, safe upstream status/code, and every fallback without exposing prompts or keys. Do not put API keys in source files or commit them. Provider settings are `XCHAT_PROVIDER=bedrock|gemini|mistral|groq|omniroute|none`, `AWS_BEDROCK_MODEL=nvidia.nemotron-nano-3-30b`, `GEMINI_MODEL=gemini-3.8-flash`, `MISTRAL_MODEL=mistral-small-latest`, `GROQ_MODEL=openai/gpt-oss-20b`, `CEREBRAS_MODEL=gpt-oss-120b`, and `OMNIROUTE_MODEL=auto`, with corresponding timeout/max-retry variables. Bedrock uses the OpenAI-compatible Runtime endpoint in `ap-south-1` and reads its bearer key from `AWS_BEARER_TOKEN_BEDROCK`. Groq defaults to `GROQ_REASONING_EFFORT=low` and `GROQ_MAX_OUTPUT_TOKENS=450`; XChat also narrows clear questions to relevant entitled tools while retaining the full permitted tool set for ambiguous questions. Conversation settings remain `XCHAT_HISTORY_DAYS=30` and `XCHAT_MAX_MESSAGES=200`.

Super Admin can set XChat billing vendor-wise to either **Fixed token credits** or **Pay as you go**, and set the selling price per 1,000 tokens. Every provider-reported chatbot input and output token is stored in the usage ledger and rolled into the vendor subscription counters. The vendor editor shows total consumption value, already-invoiced tokens, and the next unbilled XChat charge. Invoice generation includes that charge and advances a billing watermark so the same tokens cannot be invoiced twice. Fixed-credit vendors are blocked from starting another chatbot request once their total reaches the configured limit; pay-as-you-go vendors remain active while cumulative usage is tracked for billing.

People and faculty CSV/XLS/XLSX imports use one optional LLM call to map arbitrary headers to canonical fields such as name, phone, employee/student ID, email, department, designation, shift, and class metadata. The model receives only header names and aggregate value-shape statistics, never spreadsheet cell values. Model output is restricted to exact uploaded headers and known fields, then checked against deterministic mappings. If the name remains ambiguous, the People UI asks the operator to confirm the mapping before importing any rows. Mapping-call usage is tagged separately in the internal ledger and is excluded from vendor chatbot credits and invoice charges. Set `SPREADSHEET_LLM_MAPPING_ENABLED=false` to use deterministic mapping only.
...
For resilient XChat routing, copy `ai-provider-keys.env.example` to `ai-provider-keys.env` on the EC2 instance and add any available keys. The real file is ignored by Git and the installer forces mode `0600`; use unquoted `KEY=value` entries and never paste the file into logs or chat. Currently supported fallbacks are Groq, Gemini, and Cerebras:
...
When OmniRoute is selected, `setup_aws.sh` securely copies present keys into the active server environment, verifies each fallback without making invalid credentials fatal to the deployment, and configures `OmniRoute -> Groq -> Gemini -> Cerebras`. Missing providers are omitted. Every new user message starts with OmniRoute `auto`; if it fails, times out, or reaches quota, XChat advances to the next configured provider. The provider that succeeds remains selected for the rest of that message's tool-call rounds, preventing repeated waits and keeping the exchange coherent. Application-level orchestration uses zero same-provider retries by default because the next provider is the retry. Existing standalone Gemini, Mistral, and Groq modes retain their own retry settings.
