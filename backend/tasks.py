...
def process_registration_batch_items(batch_id, vendor_id, params):
    ...
    # Get pending items
    c.execute("SELECT id, image_b64 FROM registration_batch_items WHERE batch_id = ? AND status = 'pending' ORDER BY seq ASC", (batch_id,))
    items = c.fetchall()
    
    print(f"[CELERY] process_registration_batch_items: batch {batch_id} has {len(items)} items pending.", flush=True)

    for item in items:
        item_id, img_b64 = item[0], item[1]
        try:
            # Update status to processing
            c.execute("UPDATE registration_batch_items SET status = 'processing' WHERE id = ?", (item_id,))
            conn.commit()
            ...
            print(f"[CELERY] process_registration_batch_items: processed item {item_id} in {t1-t0:.3f}s. Found {len(faces)} faces.", flush=True)
            ...
            print(f"[CELERY] process_registration_batch_items: Error on item {item_id}: {e}", flush=True)
            # Mark as failed
            c.execute(
    ...
    # Check if all items are done/failed, then mark batch completed
    c.execute("SELECT COUNT(*) FROM registration_batch_items WHERE batch_id = ? AND status IN ('pending', 'processing')", (batch_id,))
    remaining = c.fetchone()[0]
    if remaining == 0:
        c.execute("UPDATE registration_batches SET status = 'completed' WHERE id = ?", (batch_id,))
        conn.commit()
