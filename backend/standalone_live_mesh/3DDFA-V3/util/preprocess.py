...
def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    ...
    # Ensure translation vector is 1D scalars
    try:
        t = np.array([float(t[0]), float(t[1])], dtype=np.float32)
    except Exception:
        t = np.array([float(t[0][0]), float(t[1][0])], dtype=np.float32)
...
def load_lm3d():
    ...
    mat_path = os.path.join(os.path.dirname(__file__), '../assets/similarity_Lm3D_all.mat')
