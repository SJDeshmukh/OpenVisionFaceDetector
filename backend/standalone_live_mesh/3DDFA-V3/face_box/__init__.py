...
class retinaface:
    def __init__(self, device):
        ...
        model_path = os.path.join(os.path.dirname(__file__), '../assets/large_base_net.pth')
        self.landmark_model = LargeModelInfer(model_path, device=device)
...
class mtcnnface:
    def __init__(self):
        if MTCNN is None:
            raise RuntimeError("mtcnn is not installed")
    ...
    def detector(self, im):
        ...
        if len(facial_landmarks)>0:
            ...
            if highest_confidence_landmark['confidence'] > 0.6:
                ...
                for k in keys:
                    pos = highest_confidence_landmark['keypoints'][k]
