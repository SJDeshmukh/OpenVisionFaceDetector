...
class face_model:
    def __init__(self, args):
        ...
        model = np.load(os.path.join(os.path.dirname(__file__), "../assets/face_model.npy"),allow_pickle=True).item()
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), "../assets/net_recon.pth"), map_location=torch.device('cpu'))['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), "../assets/net_recon_mbnet.pth"), map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
    ...
    def forward(self, no_pca = False):
        ...
        if self.args.extractTex:
            ...
            if no_pca:
                ...
                # use pca-texture for texture blending with smoothed mask
                mask = ((1 - uv_weight.detach().cpu().permute(0, 2, 3, 1).numpy()[0])*255)[:,:,0].astype(np.uint8)
                mask = cv2.GaussianBlur(mask, (7, 7), 0) # Smooth mask to reduce seams
                res_colors = cv2.seamlessClone((uv_color_img*255).astype(np.uint8), (uv_color_pca*255).astype(np.uint8), mask, (512, 512), cv2.NORMAL_CLONE) / 255.
