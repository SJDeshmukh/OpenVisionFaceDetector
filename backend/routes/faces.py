...
from services.face_service import _ensure_vendor_emb_cache, _normalize_vec, _suggest_from_cache, process_embedding, _detect_faces_from_bytes
...
def detect_faces_basic():
    ...
    try:
        ...
        crop_mode = (data.get('crop_mode') if 'data' in locals() and isinstance(data, dict) else None) or "Tight"
        lr = LOW_RAM_MODE
        if lr:
        ...
        if isinstance(annotated, tuple) and len(annotated) == 2:
            ...
            for i, (box, score_str) in enumerate(anns or []):
                ...
                if i < len(crops):
                    ...
                    # Use a balanced centered box for the thumbnail crop (1.5x tight for embedding consistency)
                    cx1, cy1, cx2, cy2 = mfd_app._compute_centered_box(bx1, by1, bx2, by2, iw, ih, scale=1.5)
                    ...
                    try:
                        if pure_face.size > 0:
                            ...
                            if landmarks_3d:
                                try:
                                    ...
                                    pass
                            pure_face = mfd_app.get_gfpgan_manager().enhance_crop(pure_face, fidelity=0.8, upscale=2, landmarks=lmks_local)
                        ...
                        emb_norm = process_embedding(emb)
                        if emb_norm.size > 0:
...
def upload_face():
    ...
    try:
        ...
        # Pre-capture image bytes if possible for later extraction
        raw_img_bytes = None
        if face_image and isinstance(face_image, str):
            try:
                if face_image.startswith('data:image'):
                    raw_img_bytes = base64.b64decode(face_image.split(',', 1)[1])
                elif not face_image.startswith('http'):
                    raw_img_bytes = base64.b64decode(face_image)
            except Exception:
                pass
        ...
        if person_id:
            ...
            if not fields:
                ...
                if (face_image is not None and face_image != "") or (templates_list and len(templates_list) > 0) or (templates is not None and templates != ""):
                    try:
                        ...
                        # Unified Persistence: Prefer extracting from image, then fallback to templates
                        persist_done = False
                        if raw_img_bytes:
                            p_params = {"crop_mode": "Tight", "enhancer": "GFPGAN", "gfpgan_upscale": 1}
                            res_faces, _ = _detect_faces_from_bytes(raw_img_bytes, p_params, vendor_id or existing.get("vendor_id"))
                            print(f"[DEBUG] upload_face (Update) - res_faces count: {len(res_faces)}", flush=True)
                            if res_faces:
                                f0 = res_faces[0]
                                print(f"[DEBUG] upload_face (Update) - f0 keys: {f0.keys()}", flush=True)
                                print(f"[DEBUG] upload_face (Update) - landmarks_3d len: {len(f0.get('landmarks_3d', []))}", flush=True)
                                vec_blob = base64.b64decode(f0['emb_vec']) if f0.get('emb_vec') else None
                                struct_blob = base64.b64decode(f0['struct_vec']) if f0.get('struct_vec') else None
                                lmks_json = json.dumps(f0.get('landmarks_3d')) if f0.get('landmarks_3d') else None
                                if vec_blob:
                                    c.execute("""INSERT INTO person_embeddings (vendor_id, person_id, vec, dim, struct_vec, landmarks_3d) 
                                                 VALUES (?, ?, ?, ?, ?, ?)""", 
                                              (vendor_id or existing.get("vendor_id"), person_id, vec_blob, len(vec_blob)//4, struct_blob, lmks_json))
                                    persist_done = True
            ...
            try:
                ...
                if raw_img_bytes:
                    p_params = {"crop_mode": "Tight", "enhancer": "GFPGAN", "gfpgan_upscale": 1}
                    res_faces, _ = _detect_faces_from_bytes(raw_img_bytes, p_params, vendor_id)
                    print(f"[DEBUG] upload_face (Insert) - res_faces count: {len(res_faces)}", flush=True)
                    if res_faces:
                        f0 = res_faces[0]
                        print(f"[DEBUG] upload_face (Insert) - f0 keys: {f0.keys()}", flush=True)
                        print(f"[DEBUG] upload_face (Insert) - landmarks_3d len: {len(f0.get('landmarks_3d', []))}", flush=True)
                        vec_blob = base64.b64decode(f0['emb_vec']) if f0.get('emb_vec') else None
                        struct_blob = base64.b64decode(f0['struct_vec']) if f0.get('struct_vec') else None
                        lmks_json = json.dumps(f0.get('landmarks_3d')) if f0.get('landmarks_3d') else None
                        if vec_blob:
                            c.execute("""INSERT INTO person_embeddings (vendor_id, person_id, vec, dim, struct_vec, landmarks_3d) 
                                         VALUES (?, ?, ?, ?, ?, ?)""", 
                                      (vendor_id, new_id, vec_blob, len(vec_blob)//4, struct_blob, lmks_json))
                            persist_done = True
