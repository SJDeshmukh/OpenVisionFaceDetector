...
class EmbeddingProcessor:
    """
    Centralized hook for custom embedding transformations.
    User can modify the 'transform' method to apply custom algorithms
    (e.g., projection matrices, dimension reduction, or distance maximization).
    """
    @staticmethod
    def transform(vec: np.ndarray) -> np.ndarray:
        """
        Custom algorithm to transform embeddings.
        Example: apply a non-linear mapping or projection.
        """
        # Placeholder for user's custom algorithm
        # Note: Input is already normalized unit vector.
        return vec

def process_embedding(v: np.ndarray) -> np.ndarray:
    """
    Standard pipeline: Normalize -> Custom Transform -> Re-normalize.
    Ensures embeddings are ready for cosine similarity / FAISS IndexFlatIP.
    """
    if v is None or v.size == 0:
        return np.zeros((0,), dtype=np.float32)
    ...
    # 2. Custom User Transformation
    v = EmbeddingProcessor.transform(v)
    
    # 3. Final normalization to maintain unit length
...
def _detect_faces_from_bytes(image_bytes: bytes, params: dict, vendor_id):
    try:
        ...
        crop_mode = params.get('crop_mode') or "Tight"
        lr = LOW_RAM_MODE
        ...
        if isinstance(annotated, tuple) and len(annotated) == 2:
            ...
            for i, (box, score_str) in enumerate(anns or []):
                ...
                if i < len(crops):
                    ...
                    cx1, cy1, cx2, cy2 = mfd_app._compute_centered_box(bx1, by1, bx2, by2, img_rgb.shape[1], img_rgb.shape[0], scale=1.5)
